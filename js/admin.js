...
(function () {
  ...
  function loadCarouselEditor() {
    ...
    if (slides.length === 0) {
      listEl.innerHTML = '<div class="empty-state"><div class="empty-icon">🖼️</div><p>No slides yet. Add your first slide below.</p></div>';
      return;
    }
  ...
  function loadHeadlinesEditor() {
    ...
    if (headlines.length === 0) {
      listEl.innerHTML = '<div class="empty-state"><div class="empty-icon">📢</div><p>No status messages. Add one below.</p></div>';
      return;
    }
  ...
  function loadEventsEditor() {
    ...
    if (events.length === 0) {
      listEl.innerHTML = '<div class="empty-state"><div class="empty-icon">📅</div><p>No events yet. Add your first event below.</p></div>';
      return;
    }

    // Sort by date for display
    const sorted = events.map((e, i) => ({ ...e, _idx: i })).sort((a, b) => new Date(a.date) - new Date(b.date));
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (addHeadlineBtn) addHeadlineBtn.addEventListener('click', handleAddHeadline);
    ...
    if (viewDashBtn) viewDashBtn.addEventListener('click', () => window.open('/', '_blank'));
  });
